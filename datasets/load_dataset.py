...
def read_keel_dat(file_path: str):
    ...
    """
    try:
        with open(f"{file_path}.dat", 'r') as file:
...
def read_json_dataset(dataset_name: str, dataset_path: Path, normalize: bool = False) -> list[
    tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]]:
    data = read_keel_dat(dataset_path.parent/dataset_name)
    if normalize:
        datan = minmax_scale(data.data,axis=0)
        targetn = minmax_scale(data.target, axis=0)
        # Target names are the same than target in regression, I've not normalized them
        data = Dataset(datan, data.feature_names, targetn, data.target_names)
...
def generate_one_client(clients: list[tuple[np.ndarray,np.ndarray,np.ndarray,np.ndarray]]) -> tuple[np.ndarray,np.ndarray,np.ndarray,np.ndarray]:
    X_train = clients[0][0]
    X_test = clients[0][1]
    y_train = clients[0][2]
    y_test = clients[0][3]
